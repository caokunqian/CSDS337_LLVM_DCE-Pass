...
%token ID BOOL_TYPE INT_TYPE FLOAT_TYPE STRING_TYPE VOID_TYPE SEMICOLON FOR LPAREN RPAREN COMMA LBRACE RBRACE IF ELSE WHILE BREAK RETURN EQUALS_SIGN LOGICAL_OR LOGICAL_AND LOGICAL_NOT RELOP_GT RELOP_LT RELOP_GE RELOP_LE RELOP_EQ RELOP_NE ARITH_PLUS ARITH_MINUS ARITH_MULT ARITH_DIV ARITH_MOD VARIADIC BOOL_LITERAL INT_LITERAL FLOAT_LITERAL STRING_LITERAL EOL

%type <boolval> BOOL_LITERAL bool_lit
...
funDef: type ID LPAREN params RPAREN LBRACE varDecs stmts RBRACE {
  ...
  auto f = ast.AddFunction($2, std::unique_ptr<VarType>($1), std::move(params), variadic);
